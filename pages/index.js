  ...
  */
  ...
  const joinGame = (game_uid) => {
    if (game_uid && profile) {
      ...
      axios
        .post("/api/join", { game_uid: game_uid, userid: profile.id })
        .then((res) => {
    ...
    <div className="p-2 flex flex-col justify-center items-center space-y-4">
      <div className="flex flex-col gap-8 my-4">
        <div className="w-full flex flex-row justify-between items-center space-x-4">
          <button
            onClick={() => createGame()}
            className="h-12 border px-4 py-2 rounded-full w-32"
          >
        ...
        {games ? (
          <ul role="list" className="divide-y divide-gray-200 p-4">
            {games.map(function (game, index) {
              return (
                <li className={hostedGameStyle(game.host)} key={index}>
                  ...
                  <div className="flex flex-col justify-between items-center space-y-4">
                    <button
                      className="px-4 py-2 rounded-full w-32 bg-green-800 text-white"
                      onClick={() => joinGame(game.game_uid)}
                    >
                      Join
                    </button>
                    <button
                      className="px-4 py-2 rounded-full w-32 bg-green-800 text-white"
                      onClick={() => closeGame(game.game_uid)}
                    >
                      Close
                    </button>
                  </div>
